...
def images_to_npy(Dataset1,label_file,classes,input_height,input_width,input_chans,resize,normalize,one_hot,compress,output_file):
  ...
  for i in range(len(listImages)):
    ...
    img = cv2.imread(os.path.join(Dataset1,image_name+".png"))
...
def run_main():
  ...
  ap.add_argument('-id','--Dataset1',    type=str,   default='Dataset1', help='Full path of folder containing images.')
  ...
  ap.add_argument('-o', '--output_file',  type=str,   default='dataset1.npz', help='Full path of output file.')
  args = ap.parse_args()
  ...
  images_to_npy(args.Dataset1, args.label_file, args.classes, args.input_height, \
